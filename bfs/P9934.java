...
public class P9934 {
	...
	static class Graph {
		...
		public void levelOrder() {
			// 구현하면 되겠죠
			// 큐
			int flag = 1; // count가 flag랑 같아지면 개행을 해라 라는 기준선
			int count = 0;
	...
	static Node makeTree(int start, int end, int depth) {
		// start = 0
		// end = 6
		if (start == end) return new Node(arr[start], depth);
		Node root = new Node(arr[(end - start) / 2], depth);
		root.left = makeTree(start, (end - start) / 2 - 1, depth + 1);
		root.right = makeTree((end - start) / 2 + 1, end, depth + 1);
		return root;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		graph.root = makeTree(0, arr.length - 1, 1);
