...
public class F2019 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < n; ++i) {
			if (rstn() == 3) three.add(rstn());
			else five.add(rstn());
		}
		three.sort(Comparator.reverseOrder());
		five.sort(Comparator.reverseOrder());
		long[] threeArr = new long[three.size() + 1];
		long[] fiveArr = new long[five.size() + 1];

		for (int i = 0; i < three.size(); ++i) threeArr[i + 1] = threeArr[i] + three.get(i);
		for (int i = 0; i < five.size(); ++i) fiveArr[i + 1] = fiveArr[i] + five.get(i);

		long ans = 0;
		for (int i = 0; i * 3 <= m; ++i) ans = max(threeArr[min(i, three.size())] + fiveArr[min(five.size(), (m - i * 3) / 5)], ans);
