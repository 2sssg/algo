...
public class B2021 {
	...
	public static void main(String[] args) throws IOException {
		n = rn();
		pq = new PriorityQueue<>((o1, o2) -> o1.x == o2.x ? o1.y - o2.y : o1.x - o2.x);
		for (int i = 0; i < n; ++i) {
			int s = rstn(), e = rstn();
			pq.add(new Pair(s, e));
		}
		...
		while (!pq.isEmpty()) {
			Pair p = pq.poll();
			if (list.get(idx).x <= p.x && p.x <= list.get(idx).y) {
				list.get(idx).y = max(list.get(idx).y, p.y);
			} else {
				list.add(p);
//				idx++;
			}
		...
		int can = 2 * list.get(0).y;
		int ans = list.get(0).y;
		for (int i = 1; i < list.size(); ++i) {
			Pair cur = list.get(i);
			if (cur.x > can) break;
			can = max(can, 2 * cur.y - cur.x);
			ans = max(ans, list.get(i).y);
		}
