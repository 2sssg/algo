...
public class P17501 {
	static final int MINUS = -100_001;
	static final int PLUS = 100_001;
	static int n, minusCount, ans;
	static int[] arr;
	static int[][] tree;

	static void init() throws IOException {
		arr = new int[n = rn()];
		tree = new int[2 * n - 1][3];
		minusCount = 0; ans = 0;

		for (int i = 0; i < 2 * n - 1; ++i) Arrays.fill(tree[i], -IINF);
		for (int i = 0; i < n; ++i) {
			arr[i] = rn();
			tree[i][0] = arr[i];
		}
		for (int i = n; i < 2 * n - 1; ++i) {
			...
			int l = rstn() - 1, r = rstn() - 1;
			if (type.equals("-")) tree[i][0] = MINUS;
			else tree[i][0] = PLUS;
			tree[i][1] = l;
			tree[i][2] = r;
		}
	...
	static void dfs(int cur, boolean isMinus) {
		if (tree[cur][1] == -IINF && tree[cur][2] == -IINF) {
			minusCount += isMinus ? 1 : 0;
			return ;
		}
		dfs(tree[cur][1], isMinus);
		if (tree[cur][0] == MINUS) isMinus ^= true;
		dfs(tree[cur][2], isMinus);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		Arrays.sort(arr);
		dfs(2 * n - 2, false);
		for (int i = 0; i < minusCount; ++i) ans += (-arr[i]);
		for (int i = minusCount; i < n; ++i) ans += arr[i];
