...
public class P20182 {
	...
	static int[] dist;
	static PriorityQueue<Triple> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o.z));
	...
	public static void main(String[] args) throws IOException {
		...
		while (!pq.isEmpty()) {
			Triple cur = pq.poll();
			if (dist[cur.x] < cur.z) continue;
			if (cur.x == b) {
				pt(cur.z);
				return ;
			}
			for (Pair nxt : adjList[cur.x]) {
				int maxCost = max(cur.z, nxt.y);
				if (dist[nxt.x] < maxCost) continue;
				if (cur.y + nxt.y > c) continue;
				pq.add(new Triple(nxt.x, cur.y + nxt.y, maxCost));
				dist[nxt.x] = maxCost;
			}
		}
		pt(-1);
...
//    static void testPrint(Pair[] arr) {for (int i = 0; i < arr.length; ++i) System.out.printf("{x : %d, y : %d}, ", arr[i].x, arr[i].y);}
	...
	static class Triple {int x;int y;int z;public Triple(int x, int y,int z) {this.x = x;this.y = y;this.z = z;}public String toString() {return "Triple{"+"x="+x+", y="+y+", z="+z+'}';}}
