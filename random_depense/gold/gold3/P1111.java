...
public class P1111 {
	...
	public static void main(String[] args) throws IOException {
		...
		if (n == 1 || (n == 2 && arr[0] != arr[1])) sb.append("A");
		else if (n == 2) sb.append(arr[0]);
		else {
			int a = arr[0] == arr[1] ? 1 : (arr[2] - arr[1]) / (arr[1] - arr[0]), b = arr[0] == arr[1] ? 0 : arr[1] - (arr[0] * a);
			for (; idx < n; idx++) if (arr[idx] != (arr[idx - 1] * a + b)) break;
			sb.append(idx != n ? "B" : arr[n - 1] * a + b);
		}
		pt(sb);
	}
