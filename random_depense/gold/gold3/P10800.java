...
public class P10800 {
	static int[] ans, c, s;
	static int n;
	static List<Triple<Integer, Integer, Integer>> list;
	public static void main(String[] args) throws IOException {
		...
		s = new int[200005];
		list = new ArrayList<>();
		for(int i = 0; i < n; i++) list.add(new Triple<>(rstn(),rstn(),i));
		list.sort((o1, o2) -> {
		...
		for(int i = 0; i < n; i++){
			...
			int weight = list.get(i).y;
			int color = list.get(i).x;
			int idx = list.get(i).z;
			...
			if (i != 0 && (list.get(i - 1).x == color) && (list.get(i - 1).y == weight))
				ans[idx] = ans[list.get(i - 1).z];
		}
		for (int i = 0; i < n; i++)
