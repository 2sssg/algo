...
public class P16657 {
	...
	public static boolean isWin(int now) {
		for (int i = 0; i < 3; i++) if (arr[i][0] == now && arr[i][0] == arr[i][1] && arr[i][1] == arr[i][2]) return true;
		for (int j = 0; j < 3; j++) if (arr[0][j] == now && arr[0][j] == arr[1][j] && arr[1][j] == arr[2][j]) return true;
		if (arr[0][0] == now && arr[0][0] == arr[1][1] && arr[1][1] == arr[2][2]) return true;
		return arr[0][2] == now && arr[0][2] == arr[1][1] && arr[1][1] == arr[2][0];
	}
	...
	public static int game(int now) {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (arr[i][j] == 0) {
					arr[i][j] = now;
					if (isWin(now)) min = min(min, -1);
					min = min(min, game(now == 1 ? 2 : 1));
					arr[i][j] = 0;
				}
			}
		}
	...
	public static void main(String[] args) throws IOException {
		int zero=0;
		arr[0] = ra(); arr[1] = ra(); arr[2] = ra();
		for (int i = 0; i < 3; i++) for (int j = 0; j < 3; j++) if (arr[i][j] == 0) zero++;
		int win = game((zero & 1) == 1 ? 1 : 2);
		System.out.println(win == 1 ? 'W' : (win == 0 ? 'D' : 'L'));
	}
