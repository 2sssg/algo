...
public class P2228 {
	...
	public static void main(String[] args) throws Exception {
		...
		dp = new int[n + 1][m + 1];
		carr = carr(n);
		for (int i = 0; i <= n; i++) for (int j = 1; j <= m; j++) dp[i][j] = -HIINF;
		dp[1][1] = carr[1];
		for (int i = 2; i <= n; i++) for (int j = 1; j <= m && (dp[i][j] = dp[i - 1][j]) >= -IINF; j++) for (int k = i - 2; k >= (j == 1 ? -1 : 0); k--) dp[i][j] = max(dp[i][j], carr[i] + (k < 0 ? 0 : dp[k][j - 1] - carr[k + 1]));
		pt(dp[n][m]);
	}
