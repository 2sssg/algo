...
public class P9470 {
	static int t, k, m, p;
	static int[] arr;
	static Pair<Integer, Integer>[] strahler;
	static List<Integer>[] adjList;
	public static void main(String[] args) throws IOException {
		br = Source.getBufferedReader();
		t = rn();
		while (t-- > 0) {
			k = rstn(); m = rstn(); p = rstn();
			arr = new int[m + 1];
			adjList = new ArrayList[m + 1];
			strahler = new Pair[m + 1];
			for (int i = 0; i <= m; ++i) {
				adjList[i] = new ArrayList<>();
				strahler[i] = new Pair<>(0, 0);
			}
			for (int i = 0; i < p; ++i) {
				int u = rstn(), v = rstn();
				arr[v]++;
			...
			while (!q.isEmpty()) {
				...
				for (int nxt : adjList[cur]) {
					arr[nxt]--;
					if (Objects.equals(strahler[nxt].x, strahler[cur].x)) {
						strahler[nxt].y++;
					} else if (strahler[nxt].x < strahler[cur].x) {
						strahler[nxt].x = strahler[cur].x;
						strahler[nxt].y = 1;
					}
					if (arr[nxt] == 0) {
						q.add(nxt);
			...
			sb.append(k).append(" ").append(strahler[m].x).append("\n");
		}
		pt(sb);
