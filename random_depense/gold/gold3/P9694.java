...
public class P9694 {
	...
	static int[] dist;
	static int[] path;
	static StringBuilder tempBuilder = new StringBuilder();
//	static PriorityQueue<Pair> pq;
	...
	static void dijkstra(int CASE) {

		PriorityQueue<Pair> pq = new PriorityQueue<>(Comparator.comparingInt(n -> n.y));
		...
		while(!pq.isEmpty()) {
			...
			for (int i = 0; i < adjList[cur].size(); i++) {
				int next =  adjList[cur].get(i).x;
				int nextW =  adjList[cur].get(i).y;
...
//			int cur = n - 1;
//			while (cur != -1) {
//				tempBuilder.append(cur).append(" ");
//				cur = path[cur];
//			}
//			tempBuilder.setLength(tempBuilder.length() - 1);
//			sb.append(tempBuilder.reverse());
//			System.out.println(sb);
//			sb.setLength(0);
//		}
...
//			tempBuilder.setLength(0);
//			for (int i = (n-1); i > 0;) {
//				tempBuilder.append(i = path[i]).append(" ");
//			}
...
//			while (!pq.isEmpty()) {
//				Pair cur = pq.poll();
//				if (cur.y > dist[cur.x][0]) continue;
//				if (cur.x == n - 1) break;
//				for (Pair nxt : adjList[cur.x]) {
//					int nx = nxt.x;
//					int ny = cur.y + nxt.y;
//					if (dist[nx][0] < ny) continue;
//					if (dist[nx][1] <= dist[cur.x][1]) continue;
//					if (dist[cur.x][1] == 0) {
//						dist[nx][1] = nxt.y;
//					} else {
//						dist[nx][1] = dist[cur.x][1];
//					}
//					dist[nx][0] = ny;
//					path[nx] = cur.x;
...
//			if (dist[n - 1][0] == HIINF) sb.append(" -1\n");
//			else {
//				StringBuilder tempBuilder = new StringBuilder(" ");
//				int cur = n - 1;
//				sb.append(" 0");
...
//					cur = path[cur];
//				}
//				sb.append(tempBuilder.reverse()).append("\n");
//			}
			dijkstra(c);
		}
...
//    static void testPrint(Pair[] arr) {for (int i = 0; i < arr.length; ++i) System.out.printf("{x : %d, y : %d}, ", arr[i].x, arr[i].y);}
	...
	////////////////////////////// 자료구조 ///////////////////////////////////////////
	static class Pair {int x;int y;public Pair(int x, int y) {this.x = x;this.y = y;}public String toString() {return "Pair{x="+x+", y="+y+'}';}}
