...
public class P7573 {
	...
	static ArrayList<Pair<Integer, Integer>> fishs = new ArrayList<>();

	public static void main(String[] args) throws IOException {
		n = rstn(); l = rstn(); m = rstn();

		for (int i = 0; i < m; i++) fishs.add(new Pair<>(rstn(), rstn()));
			...
			for (int j = 0; j < m; j++) {
				for (int k = 1; k < l / 2; k++) {
					...
					for (int kk = 0; kk < m; kk++)
						if(fishs.get(i).x <= fishs.get(kk).x && fishs.get(kk).x <= fishs.get(i).x + k && fishs.get(j).y <= fishs.get(kk).y && fishs.get(kk).y <= fishs.get(j).y + (l / 2 - k))
							temp++;
					ans = max(ans, temp);
				}
			}
