package random_depense.gold.gold4;
...
public class P15971 {
	...
	static int[] dist;
	static PriorityQueue<Triple<Integer, Integer, Integer>> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o.y));

	public static void main(String[] args) throws IOException {
		n = rstn(); r1 = rstn(); r2 = rstn();
		...
		pq.add(new Triple<>(r1, 0, 0));
		dist[r1] = 0;
		int longdist = 0;
		int d = 0;
		while (!pq.isEmpty()) {
			Triple<Integer, Integer, Integer> cur = pq.poll();
			...
			for (Pair<Integer, Integer> nxt : adjList[cur.x]) {
				if (dist[nxt.x] <= cur.y + nxt.y) continue;
				Triple<Integer, Integer, Integer> next = new Triple<>(nxt.x, cur.y + nxt.y, max(cur.z, nxt.y));
				dist[nxt.x] = next.y;
				pq.add(next);
			}
		}

		System.out.println(d - longdist);
