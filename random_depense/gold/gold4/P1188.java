...
public class P1188 {
	...
	public static void main(String[] args) throws IOException {
		n = rn();
		int[] cnt = new int[26];
		PriorityQueue<Pair<Integer, Character>> pq = new PriorityQueue<>((o1, o2) -> {
			if (cnt[o1.y - 'A'] == cnt[o2.y - 'A']) return o1.x - o2.x;
			return cnt[o2.y - 'A'] - cnt[o1.y - 'A'];
		});
		for (int i = 0; i < n; ++i) {
			String method = r();
			if (method.split(" ")[0].equals("enqueue")) {
				char ch = method.split(" ")[1].charAt(0);
				cnt[ch - 'A']++;
				pq.add(new Pair<>(i, ch));
			} else {
				if (pq.isEmpty()) {
					sb.append("*, ");
					continue;
