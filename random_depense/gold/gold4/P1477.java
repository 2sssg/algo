...
public class P1477 {
	...
	static int f(int mid) {
		int ret = 0;
		for (int i = 1; i <= n + 1; ++i) ret += (arr[i] - arr[i - 1] - 1) / mid;
		return ret;
	}

	public static void main(String[] args) throws IOException {
		n = rstn(); m = rstn(); l = rstn(); arr = brab(0, l);
		Arrays.sort(arr);
		int lo = 1, hi = l;
		while (lo <= hi) {
			int mid = (lo + hi) >> 1;
	...
	static long[] brabl() throws IOException { long[] temp = ral(); long[] ret = new long[temp.length+2]; System.arraycopy(temp, 0, ret, 1, temp.length); return ret;}
