package random_depense.gold.gold5;
...
public class P2800 {
	static char[] input;
	static ArrayList<Pair<Integer, Integer>> brackets = new ArrayList<>();
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < input.length; i++) {
			...
			if (input[i] == ')') brackets.add(new Pair<>(stack.pop(), i));
		}
	...
	public static void find(int depth) {
		...
		Pair<Integer, Integer> pair = brackets.get(depth);
		visited[pair.x] = true;
		visited[pair.y] = true;
		find(depth + 1);

		visited[pair.x] = false;
		visited[pair.y] = false;
		find(depth + 1);
	}
