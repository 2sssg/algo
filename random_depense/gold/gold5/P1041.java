...
public class P1041 {
	...
	public static void main(String[] args) throws Exception {
		...
		if (n == 1) {
			...
			long min = arr[0];
			for (int i = 1; i < 6; i++) min = Math.min(min, arr[i]);
			res += num[1] * min;
			min = LINF;
			for (int i = 0; i < 6; i++) for (int j = i + 1; j < 6; j++) if (j + i != 5) min = Math.min(min, arr[i] + arr[j]);
			res += num[2] * min;
			int sum = 0;
			for (int i = 0; i < 3; i++) sum += Math.min(arr[i], arr[5 - i]);
			res += num[3] * sum;
			pt(res);
		}
	}
