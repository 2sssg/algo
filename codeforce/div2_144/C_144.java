...
public class C_144 {
	...
	static long dfs(int cur, int depth) {
		if (dp[depth].containsKey(cur)) return dp[depth].get(cur);
		if (cur * (1 << (size - 2 - depth)) > r) {
			dp[depth].put(cur, 0L);
			return 0;
		}
		if (cur > r) {
		...
		long ret = 0;
		for (int i = 2; i <= r; ++i) {
			ret += dfs(cur * i, depth + 1);
			ret %= MOD;
		}
		dp[depth].put(cur, ret);
		return ret;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			size = 1;
			for (; temp <= r; ++size) temp <<= 1;
			sb.append(size - 1).append(" ");
			long ans = 0;
			dp = new HashMap[15];
			for (int i = 0; i < 15; ++i) dp[i] = new HashMap<>();
			while (true) {
				long dtemp = dfs(l, 0);
				l++;
				if (dtemp == 0) break;
