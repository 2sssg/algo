...
public class C_874 {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			for (int item : arr) {
				if (item % 2 == 1) oddList.add(item);
				else evenList.add(item);
			}
			...
			int even = 0;
			int odd = 0;
			while (evenIdx < evenList.size() || oddIdx < oddList.size()) {
				if (evenIdx == evenList.size()) {
					if (oddIdx == 0) {
						odd++;
					} else {
						odd++;
					...
					evenIdx++;
				} else if (oddList.get(oddIdx) < evenList.get(evenIdx)) {
					if (oddIdx == 0) {
						odd++;
					} else {
						odd++;
