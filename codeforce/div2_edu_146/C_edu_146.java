...
public class C_edu_146 {

	public static void main(String[] args) throws IOException {
		br = Source.getBufferedReader();
		int t = rn();
		while (t-- > 0) {
			int n = rstn(), s1 = rstn(), s2 = rstn();
			List<Pair<Integer, Integer>> arr = new ArrayList<>();
			for (int i = 0; i < n; ++i) arr.add(new Pair<>(rstn(), i + 1));
			Collections.sort(arr, (o1, o2) -> o2.x - o1.x);
			...
			for (int i = 0; i < n; ++i) {
				...
				} else if ((a.size() + 1) * s1 < (b.size() + 1) * s2) {
					a.add(arr.get(i).y);
				} else {
					b.add(arr.get(i).y);
				}
