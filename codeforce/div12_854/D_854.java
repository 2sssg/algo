...
public class D_854 {
	...
	static int n;
	static int[] arr;
	static TreeSet<Pair> tm;

	static boolean same() {
		int temp = tm.first().y;
		for (Pair p : tm) {
			if (p.y != temp) return false;
		}
		return true;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			n = rn();
			arr = ra();
			tm = new TreeSet<>(new Comparator<Pair>() {
				@Override
			...
			for (int i = 0; i <= 30 * n; ++i) {
				...
				Pair large = tm.pollLast();

				sb.append(large.x + 1).append(" ").append(tm.first().x + 1).append("\n");
				tm.add(new Pair(large.x, (int)Math.ceil((double)large.y / (double)tm.first().y)));
			}
			if (!flag) {
				sb.setLength(0);
				sb.append("-1\n");
