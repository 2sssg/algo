...
public class D_849 {
	...
	static int getCount(int[] arr) {
		int ret = 0;
		for (int a : arr) if (a != 0) ret++;
		return ret;
	}

	static int getDifferCount(int[] arr1, int[] arr2) {
		int arr1Count = getCount(arr1);
		int arr2Count = 0;
		for (int i = 0; i < 26; ++i)
			if (arr2[i] - arr1[i] != 0)
				arr2Count++;
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			for (int i = 1; i < n; ++i) ans = max(ans, getDifferCount(cnt[i], cnt[n]));
