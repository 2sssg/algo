...
public class C_862 {
	...
	public static void main(String[] args) throws IOException {
		...
		while (t-- > 0) {
			...
			for (int i = 0; i < m; ++i) {
				long a = rstnl(), b = rstnl(), c = rstnl();
				if (min != LINF && (b - min) * (b - min) - (4 * a * c) < 0) {
					sb.append("YES\n").append(min).append("\n");
				}
				else if (max != -LINF && (b - max) * (b - max) - (4 * a * c) < 0) {
					sb.append("YES\n").append(max).append("\n");
				}
				else if (mmin != LINF && (b - mmin) * (b - mmin) - (4 * a * c) < 0) {
					sb.append("YES\n").append(mmin).append("\n");
				}
				else if (mmax != -LINF && (b - mmax) * (b - mmax) - (4 * a * c) < 0) {
					sb.append("YES\n").append(mmax).append("\n");
				}
				else sb.append("NO\n");
			}
