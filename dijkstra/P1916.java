...
public class P1916 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < m; ++i) {
			...
			weight = rstn();
			G[start].add(new Pair<>(end, weight));
		}

		PriorityQueue<Pair<Integer, Integer>> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o.y));
		int start, end;
		start = rstn();
		end = rstn();
		dist[start] = 0;
		...
		while (!pq.isEmpty()) {
			...
			if (dist[current.x] < current.y) continue;
			for (Pair<Integer, Integer> next : G[current.x]) {
				if (dist[next.x] < current.y + next.y) continue;
				dist[next.x] = current.y + next.y;
				pq.add(new Pair<>(next.x, current.y + next.y));
			}
		}
		pt(dist[end]);
	}
...
//    static void testPrint(Pair[] arr) {for (int i = 0; i < arr.length; ++i) System.out.printf("{x : %d, y : %d}, ", arr[i].x, arr[i].y);}
	...
	////////////////////////////// 자료구조 ///////////////////////////////////////////
	static class Pair<T1,T2> {T1 x;T2 y;public Pair(T1 x, T2 y) {this.x = x;this.y = y;}public String toString() {return "(x="+x+", y="+y+')';}}
