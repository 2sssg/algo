...
public class P9370 {
	...
	static List<Pair<Integer, Double>>[] adjList;
	static PriorityQueue<Pair<Integer, Double>> pq;
	static double[] dist;


	static void init() throws IOException {
		n = rstn(); m = rstn(); t = rstn(); s = rstn(); g = rstn(); h = rstn();
		adjList = new ArrayList[n + 1]; for (int i = 0; i <= n; ++i) adjList[i] = new ArrayList<>();
		for (int i = 0; i < m; ++i) {
			int u = rstn(), v = rstn(), w = rstn();
			if ((u == g && v == h) || (u == h && v == g)) {
				adjList[u].add(new Pair<>(v, (double)w - 0.1));
				adjList[v].add(new Pair<>(u, (double)w - 0.1));
			} else {
				adjList[u].add(new Pair<>(v, (double)w));
				adjList[v].add(new Pair<>(u, (double)w));
			}
		}
		dest = new HashSet<>();
		while (t-- > 0) dest.add(rstn());
		pq = new PriorityQueue<>(Comparator.comparingDouble(o -> o.y));
		dist = new double[n + 1];
	...
	static void dijk() {
		...
		while (!pq.isEmpty()) {
			...
			if (cur.y % 1 != 0.0 && dest.contains(cur.x)) answer.add(cur.x);
			for (Pair<Integer, Double> nxt : adjList[cur.x]) {
				int nx = nxt.x;
				double ny = cur.y + nxt.y;
				if (ny >= dist[nx]) continue;
				dist[nx] = ny;
		...
		for (int item : answer) sb.append(item).append(" ");
		sb.append("\n");
	}
