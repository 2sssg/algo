...
public class P1854 {
	...
	static void init() throws Exception{
		...
		for (int i = 0; i <= n; ++i) distpq[i] = new PriorityQueue<>(Collections.reverseOrder());
	...
	public static void main(String[] args) throws Exception{
		...
		while(!pq.isEmpty()) {
			...
			for(Pair nxt : adjList[cur.x]) {
				if (distpq[nxt.x].size() < k  || distpq[nxt.x].peek() > cur.y + nxt.y) {
					if(distpq[nxt.x].size() == k) distpq[nxt.x].poll();
					distpq[nxt.x].add(cur.y + nxt.y);
					pq.add(new Pair(nxt.x,cur.y + nxt.y));
				}
			}
		}
		for(int i = 1; i <= n; i++) sb.append(distpq[i].size() < k ? -1 : distpq[i].peek()).append('\n');
		pt(sb);
	}
