...
public class P20047 {
	...
	public static boolean find(int idx, int cnt) {
		if (idx + cnt == n - 1) return true;
		if (idx == n - 2) return false;

		if (s.charAt(idx) == t.charAt(idx + cnt) && find(idx + 1, cnt)) return true;
		if (cnt == 2) return false;

		return temp[cnt] == t.charAt(idx + cnt) && find(idx,cnt + 1);
	}

	public static void main(String[] args) throws IOException {
		n = rn(); s = new StringBuilder(br.readLine()); t = new StringBuilder(br.readLine()); l = rstn(); r = rstn();
		temp[0] = s.charAt(l);
		temp[1] = s.charAt(r);
		int t = 0;
		for(int i = 0; i < n; i++) {
			if (i == l || i == r) continue;
			s.setCharAt(t, s.charAt(i));
			t++;
		}
		System.out.print(find(0,0) ? "YES" : "NO");
	}
	////////////////////////////////입출력/////////////////////////////////////////////
