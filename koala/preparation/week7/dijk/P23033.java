...
public class P23033 {
	...
	static void init() throws IOException {
		...
		for(int i=0; i<m; ++i){
			int v1=rstn(),v2=rstn(),t=rstn(),w=rstn();
			adjList[v1].add(new Triple(v2,t,w));
		}
	}
	...
	static int dijk(){
		...
		while(!pq.isEmpty()){
			...
			for(Triple itm: adjList[p.x]){
				int nx = itm.x;
				int nt = p.y%itm.z==0?p.y+itm.y:p.y+(itm.z-(p.y%itm.z))+itm.y;
				if(d[nx]>nt){
					pq.add(new Pair(nx,nt));
					d[nx] = nt;
