...
public class P10270 {
	...
	static void dijk(){
		...
		while(!pq.isEmpty()){
			...
			for(Triple item: adjList[cur.ver]){
				...
				Triple nxt = new Triple(0,0,0);
				nxt.ver = item.ver;
				nxt.cost = cur.cost + item.cost;
				nxt.time = dp[cur.ver][cur.cost] + item.time;
				if(nxt.cost > m) continue;
				if(dp[nxt.ver][nxt.cost] > nxt.time){
					pq.add(nxt);
					dp[nxt.ver][nxt.cost] = nxt.time;
				}
			}
		}
	}
	...
	public static void main(String[] args) throws IOException {
		...
		while(t-->0){
			...
			bw.append(min==Integer.MAX_VALUE?"Poor KCM":String.valueOf(min)).append("\n");
//			bw.append("\n\n\n\n");
		}
		bw.flush();
	}
