...
public class P28128 {
	...
	public static void main(String[] args) throws IOException {
		n = rstn(); m = rstn();
		map = new String[n][m];
		for (int i = 0; i < n; ++i) map[i] = r().split(" ");
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < m; ++j) {
				...
				for (int k = 0; k < 4; ++k) {
					int nx = i + dx[k];
					int ny = j + dy[k];
					if (!chk(nx, ny, n, m) && cur.equals(map[nx][ny])) {
						hs.add(cur);
						continue;
					}
					nx += dx[k];
					ny += dy[k];
					if (!chk(nx, ny, n, m) && cur.equals(map[nx][ny])) hs.add(cur);
				}
				if (hs.contains(cur)) continue;
				...
				nx3 = i + 1; ny3 = j - 1;
				nx4 = i + 1; ny4 = j + 1;
				if (chk(nx1, ny1, n, m)) continue;
				if (chk(nx2, ny2, n, m)) continue;
				if (chk(nx3, ny3, n, m)) continue;
				if (chk(nx4, ny4, n, m)) continue;
				if (cur.equals(map[nx1][ny1]) && cur.equals(map[nx2][ny2]) && cur.equals(map[nx3][ny3])
						&& cur.equals(map[nx4][ny4])) hs.add(cur);
			}
		}
		List<String> answer = new ArrayList<>(hs);
		Collections.sort(answer);
		answer.forEach(item -> sb.append(item).append("\n"));
		pt(sb.length() == 0 ? "MANIPULATED" : sb);
	}
