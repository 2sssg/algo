...
public class ParityConstaintClosestPairEasy {
	static int n;
	static long[] arr;
	public static void main(String[] args) throws IOException {
		...
		long evenDist = LINF;
		long oddDist = LINF;
		if ((arr[0] & 1) == 0) even = arr[0];
		else odd = arr[0];
		for (int i = 1; i < n; ++i) {
			//짝수
			if ((arr[i] & 1) == 0) {
				if (even != -1) {
					evenDist = min(evenDist, arr[i] - even);
				}
				if (odd != -1) {
					oddDist = min(oddDist, arr[i] - odd);
				}
				even = arr[i];
			}
			// 홀수
			if ((arr[i] & 1) == 1) {
				if (odd != -1) {
					evenDist = min(evenDist, arr[i] - odd);
				}
				if (even != -1) {
					oddDist = min(oddDist, arr[i] - even);
				}
				odd = arr[i];
			}
		}
		sb.append(evenDist == LINF ? -1 : evenDist).append(" ").append(oddDist == LINF ? -1 : oddDist);
		pt(sb);
	}
