...
public class S2S {
	...
	static int f(int mid) {
		int ret = 0;
		if (Math.abs(arr[1] - arr[0]) > mid) ret++;
//		pt("ret : " + ret);
		if (n > 2) if (Math.abs(arr[n - 1] - arr[n - 2]) > mid) ret++;
//		pt("ret : " + ret);
		for (int i = 1; i < n - 1; ++i) {
			int temp = max(Math.abs(arr[i] - arr[i - 1]), Math.abs(arr[i + 1] - arr[i]));
			if (temp > mid) ret++;
		}
//		pt("ret : " + ret);
		return ret;
	...
	public static void main(String[] args) throws IOException {
		...
		int h = 0;
		for (int i = 1; i < n; ++i) h = max(h, Math.abs(arr[i] - arr[i - 1]));
		int lo = -1, hi = h;
