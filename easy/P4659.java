...
public class P4659 {
	...
	static int f(char c1, char c2) {
		if (c1 == c2 && c1 != 'e' && c1 != 'o') return -1;
		if (isConsonant(c1) && isConsonant(c2)) return 1;
		if (!isConsonant(c1) && !isConsonant(c2)) return 1;
		return 0;
	}

	public static void main(String[] args) throws IOException {
		l: while (!(s = r()).equals("end")) {
			arr = new int[s.length()];
			boolean flag = isConsonant(s.charAt(0));
			for (int i = 1; i < s.length(); ++i) {
				flag |= isConsonant(s.charAt(i));
				int f = f(s.charAt(i - 1), s.charAt(i));
				if (f == -1) {
			...
			if (!flag)  sb.append("<").append(s).append("> is not acceptable.\n");
			else sb.append("<").append(s).append("> is acceptable.\n");
		}
