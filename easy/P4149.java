...
public class P4149 {
	...
	public static List<BigInteger> ans = new ArrayList<>();
	public static Random random = new Random();
	public static BigInteger two = new BigInteger("2");

	private static void getData(BigInteger num){
		if(num.equals(BigInteger.ONE)) return;
		if(num.isProbablePrime(11)){
			ans.add(num);
			return;
		}

		BigInteger factor = rho(num);
		getData(factor);
		getData(num.divide(factor));
	...
	private static BigInteger rho(BigInteger n) {
		...
		while(factor.compareTo(BigInteger.ONE) == 0){
			a = a.multiply(a).add(c).mod(n);
			b = b.multiply(b).add(c).mod(n);
			b = b.multiply(b).add(c).mod(n);
			factor = a.subtract(b).abs().gcd(n);
		}
		return factor;
	}
	...
	public static void main(String[] arg) throws IOException {
		number = new BigInteger(r());
		getData(number);

		Collections.sort(ans);
		for(BigInteger temp : ans) sb.append(temp).append("\n");
